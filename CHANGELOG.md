...
- **`rocminfo` / HIP apps fail with `HSA_STATUS_ERROR_INVALID_ARGUMENT` after XRT install.** Root cause: `_xrtb_setup_bashrc` (Section 11b) was prepending `/lib/x86_64-linux-gnu` to `LD_LIBRARY_PATH` before sourcing `/opt/xilinx/xrt/setup.sh`. Since `LD_LIBRARY_PATH` takes precedence over a binary's embedded `RUNPATH`, all ROCm tools (`rocminfo`, `clinfo`, HIP applications) ended up loading the Ubuntu-packaged `libhsa-runtime64` from `/lib/x86_64-linux-gnu` instead of the ROCm 7.x version at `/opt/rocm/lib` — producing `HSA_STATUS_ERROR_INVALID_ARGUMENT` and an apparently empty GPU device list for non-root users. Running as root happened to work because root inherited a different environment.

  Confirmed: GPU (gfx1150 / Radeon 890M), CPU, and NPU (aie2p / Strix) are all visible in `rocminfo` once the library path is correct.

  Three changes:

  1. **`_xrtb_setup_bashrc`** — removed `export LD_LIBRARY_PATH=/lib/x86_64-linux-gnu...` entirely (the system linker searches that path by default via `/etc/ld.so.conf.d/`; it never needed to be explicit) and added `export LD_LIBRARY_PATH=/opt/rocm/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}` after the XRT source line so ROCm's libs always take precedence.

  2. **Migration path** — if the old broken line is detected in `~/.bashrc` during re-run of `--section 11b`, it is removed via `sed` and the `/opt/rocm/lib` priority line is appended. Users on v0.3.10 and earlier can fix manually:
     ```bash
     ...
     ```
     Or just re-run `./scripts/setup.sh --section 11b`.

  3. **`zenbook-validate`** (generated by Section 10) — XRT setup sourcing now also sets `LD_LIBRARY_PATH=/opt/rocm/lib:...` so the validate script checks ROCm correctly without requiring an interactive shell.

- **`zenbook-validate` missing ROCm and XRT validate checks.** `hdr "GPU / Vulkan"` expanded to `hdr "GPU / Vulkan / ROCm"` with a new `rocminfo` GPU agent count check. New `hdr "XRT / NPU"` section added before the per-method NPU checks: reads NPU firmware version from sysfs, runs `xrt-smi validate` and checks for at least one `PASSED` test.

- **`docs/09-xrt-stack.md` — source-build instructions were incomplete.** The old "Alternative: build XRT from source" section only showed how to build the XDNA plugin; it omitted the XRT base package build (`cpack -G DEB`) and gave wrong install instructions ("copy DEBs into the bundle dir and re-run `--with-xrt`" — which fails because the source-built plugin requires `xrt-base >= 2.23` while the EULA-gated bundle ships 2.21). Updated to:
  - Build XRT base: `./build.sh -npu -opt && cd Release && cpack -G DEB`.
  - Build plugin: `./build.sh -release`.
  - Install all in a single `apt install` transaction.
  - Loop to build DKMS for any additional installed kernels beyond the running one.
  - Marked as **recommended path** since it produces 2.23 (vs 2.21 from the EULA bundle) and requires no AMD account.
  - Added two new troubleshooting entries: `rocminfo HSA_STATUS_ERROR_INVALID_ARGUMENT` and `amdxdna mailbox -22 / disable irq` (the latter is cosmetic — the actual `disable_irq()` call was removed in kernel patch `cd740b873`, included in 6.17).

- **`docs/hardware-quirks.md` — ROCm status outdated; Chrome APT warning undocumented.**
  - ROCm row updated from "Officially unsupported" to confirmed working on ROCm 7.2.3 / gfx1150 / kernel 6.17.0-20.
  - New "Known workarounds" section with Chrome APT `N: Skipping acquire … doesn't support architecture 'i386'` fix (`Architectures: amd64` in `/etc/apt/sources.list.d/google-chrome.sources`).
